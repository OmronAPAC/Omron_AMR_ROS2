...
Last updated: `22 July 2021` By: `Zeon`
...
The `.map` file coordinate data is converted into an appropriately sized OccupancyGrid map. To ensure correct scaling, the map coordinates are based off the input data from the `.map` file exactly and has its origin position set to the minimum coordinates found in the `.map` file. More information on how this implementation works can be found in `amr_visualisation/src/data_points_marker.cpp`
