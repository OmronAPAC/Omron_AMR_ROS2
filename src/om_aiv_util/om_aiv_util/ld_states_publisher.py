...
class LdStatePublisher(Node):
    def __init__(self, listener):
        ...
        self.timer = self.create_timer(self.timer_period, self.timer_callback(listener))

    def timer_callback(self, listener):
        events = listener.selector.select()
        for key, mask in events:
            listener.process_events(mask)

        pub_status(self.status_pub, listener)
        pub_laser(self.laser_pub, listener)
        pub_goals(self.goals_pub, listener)
        pub_odometer(self.odom_pub, listener)
        pub_app_fault_query(self.app_fault_query_pub, listener)
        pub_faults_get(self.faults_get_pub, listener)
        pub_query_faults(self.query_faults_pub, listener)
...
def main():
    ...
    print('IP ADDRESS IS.... ' + str(ip_address))
    #port = node.get_parameter("local_port").get_parameter_value().string_value
    port = node.declare_parameter("local_port").value
    print('LOCAL PORT IS.... ' + str(port))
